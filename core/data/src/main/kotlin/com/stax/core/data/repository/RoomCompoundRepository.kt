...
class RoomCompoundRepository(
    ...
    override fun observeAll(): Flow<List<CompoundSupply>> =
        compoundDao.observeActiveWithOpened().map { list ->
            list.map { it.compound.toDomain(it.opened) }
        }

    override fun observeById(id: Long): Flow<CompoundSupply?> =
        compoundDao.observeByIdWithOpened(id).map { row ->
