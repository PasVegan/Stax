...
3. **Concentration** (Optional) — numeric + unit picker inline ("{amount} / 1 mL"; the `per` side is not a field). **Trailing "Helper" tonal button** (`secondary-container`, `calculate` icon) → §4.6 Reconstitution Helper — on Create there is no compound to pre-select yet, so it opens the standalone calculator. Required only when `Form == Injectable AND ContainerType != Ampoule` (pre-mixed); the `Optional` badge disappears exactly when that rule makes it required.
